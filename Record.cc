...
void Record ::ComposeRecord(Schema *mySchema, char *attrValues)
{
	...
	// To keep track of the char array of attr values
	int charIndex = 0;
	...
	for (int i = 0; i < n; i++)
	{
		...
		while (1)
		{
			if (attrValues[charIndex] == '|')
				break;

			space[len] = attrValues[charIndex];
			len++;
			charIndex++;
		}
...
string Record ::GetString(Schema *mySchema)
{
